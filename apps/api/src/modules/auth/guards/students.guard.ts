...
/**
 * Guard que permite acceso a gestión de estudiantes si:
 * 1. El usuario tiene rol SUPERADMIN, ADMIN_INSTITUTIONAL o COORDINADOR, O
 * 2. El usuario tiene el permiso delegado canManageStudents en su InstitutionUser
 * 
 * NOTA: Este permiso es temporal para la fase de pruebas.
 */
@Injectable()
export class StudentsGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    return institutionUser?.isActive === true && institutionUser?.canManageStudents === true;
  }
}
