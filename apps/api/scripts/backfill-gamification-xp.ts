...
async function main() {
  ...
  for (const p of progresses) {
    ...
    for (const [slideId, a] of Object.entries(answers)) {
      ...
      if (a?.isCorrect && pts > 0) {
        await grant({
          ...
          source: 'LESSON_ACTIVITY', amount: pts, reason: `Acierto en lección: ${p.lesson.title}`,
    ...
    await grant({
      institutionId: enr.institutionId, studentId: enr.studentId, studentEnrollmentId: enr.id,
      source: 'LESSON_COMPLETE', amount: 50, reason: `Lección completada: ${p.lesson.title}`,
