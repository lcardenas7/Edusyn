...
(async()=>{
 ...
 let role='ADMIN_INSTITUTIONAL', historyFails=false, configFails=false, teacherDisabled=false;
 const mutations=[], errors=[], requests=[];
 const year={id:'year-A',year:2026,name:'Año de prueba',status:'ACTIVE',isCurrent:true,terms:[{id:'term-A',name:'Período 1'}]};
 const group={id:'group-A',name:'A',gradeId:'grade-A',grade:{id:'grade-A',name:'Primero'},campus:{id:'campus-A',name:'Sede de prueba'}};
 ...
 await context.route('**/*',async route=>{
  ...
  else if(path==='/apd/config'){body=configFails?{message:'Fallo sintético'}:{enableDifferentialSupport:true,allowTeacherAccess:!teacherDisabled};status=configFails?500:200;}
  else if(path==='/apd/workspace')body={academicYears:[year],groups:[group,group2]};
  ...
  else if(path==='/apd/plans')body=[plan];
  ...
  else if(path==='/apd/workspace/students')body=[{...student,enrollmentId:'enr-A'}];
 ...
 try{
  ...
  assert(requests.includes('/apd/workspace/students'));
  ...
  teacherDisabled=true;await open('/differential-support');
  await page.getByRole('heading',{name:'Acceso docente no habilitado'}).waitFor();
  teacherDisabled=false;role='PSICOLOGA';await open('/differential-support');
  await page.getByText('Lectura acompañada',{exact:true}).waitFor();
