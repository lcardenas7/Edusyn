...
/** En la tarjeta del equipo: aprobar lo que piden publicar, retirar la app y ver su uso. */
export default function TeacherPublication({ publication, onChanged }: { publication: ConstruyePublication; onChanged: (publication: ConstruyePublication) => void }) {
  const [busy, setBusy] = useState(false)
  const [expiresAt, setExpiresAt] = useState(publication.expiresAt ? publication.expiresAt.slice(0, 10) : '')
  ...
  const approve = () => run(() => construyePublicationApi.approve(publication.id, { expiresAt: expiresAt || null }), 'App publicada')
  const reject = async () => {
  ...
  const unpublish = async () => {
    if (!await confirmDialog('La app dejará de abrir en el enlace y en los celulares donde la instalaron. Podrás volver a publicarla con el mismo enlace.', { title: 'Retirar la app', confirmLabel: 'Retirar', danger: true })) return
  ...
  return <div className="mt-3 rounded-xl border border-indigo-100 bg-indigo-50/50 p-3">
    ...
    {pending !== null && <div className="mt-2 space-y-2 rounded-lg bg-white p-2.5 text-xs text-slate-700">
      ...
      <label className="flex flex-wrap items-center gap-2">Disponible hasta (opcional)
