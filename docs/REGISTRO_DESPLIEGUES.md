...
- **Fix pegado estilo Python:** varias IA devuelven CÓDIGO en vez de JSON (`quiz = {...}`, booleanos `True`/`False`/`None`, líneas `path=...`/`json.dump` al final) → `JSON.parse` fallaba con "No se pudo leer un JSON válido". `lib/extractJson.ts` ahora recorta el objeto y traduce literales de Python a JSON **fuera de las comillas** (respeta el texto de las preguntas). Cubre "Crear con IA" y el importador del editor de Quiz.
