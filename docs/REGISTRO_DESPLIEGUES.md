...
|-------|---------|--------|-----------|--------|
| 2026-09-17 | producción | e8f5bede + 8dd0e764 | **No** | **Aula / quiz en casa:** progreso y puntajes desde el aula nueva (antes solo desde la lista del aula antigua) y arreglos de la prueba de punta a punta local (API+web, 1 docente, 3 estudiantes): al finalizar se cierran y califican quienes no terminaron; entrada serializada (`pg_advisory_xact_lock`) sin sesiones duplicadas; «terminaron» cuenta estudiantes con todo respondido; resultados finales tras cerrar; diálogo «Finalizar»; sala de espera con contador; «+0 pts» corregido; estudiante entra directo desde «Continuar». Sin migración. API 984, web 431; api y web SUCCESS, health OK, bundle verificado. Mismo cambio que staging 885b060a + a9e01f2d. |
| 2026-09-16 | producción | 04f6dcfc | **No** | **Aula / menú lateral fijo:** en escritorio el aula ya no hereda el margen de 32 px del Layout; el riel pasa de `sticky` a `fixed` (con hueco que reserva su ancho) y no se mueve con el contenido. Solo web; web SUCCESS, API SKIPPED, bundle verificado. Mismo cambio que staging ab16d055. |
| 2026-09-16 | producción | c9604e21 + b09b44aa | **No** | **Aula / autoevaluación y coevaluación:** preguntas en la voz correcta (coevaluación sobre el compañero, con advertencia en la petición a la IA y aviso en el editor); una sola dimensión por tipo; borradores con Ver preguntas / Editar / Eliminar (`GET/PUT/DELETE formative-evaluations/:id`, solo borradores; plantillas viejas se desactivan); cuestionario por aspectos (13 aspectos con preguntas de ejemplo, propios, 1–4 por aspecto, hasta 30) y preguntas relacionadas o independientes. Mismo cambio que staging 6ef59cc9 + 59c4859b. API 982, web 431, build OK; api y web SUCCESS, health OK, bundle verificado. El arreglo del quiz en casa (staging 885b060a) sigue sin promover. |
| 2026-09-16 | producción | 4afd39d0 | **No** | **Hotfix aula / autoevaluación:** `POST formative-evaluations/from-ai-draft` daba 500 (P2002 en `AttitudinalRubric(institutionId,name)`) al repetir el título de una evaluación o repetir etiquetas de dimensión. Ahora elige un nombre libre ("… (2)"). Mismo cambio en staging 55ce2d14. API 979 pruebas OK; api SUCCESS y health OK. |
| 2026-09-16 | producción | a55a3c85 | **No** | **Aula / coevaluación:** mismo cambio que staging 7c95e371: reparto de coevaluación automático (equilibrado, con semilla) o manual antes de publicar; destino en la planilla solo al enviar resultados (opcional); coevaluación agregada copia criterios y se puede traer de IA externa; botón directo "Crear con IA externa" en Hoy/Actividades; aula a ancho completo. Sin migración. api y web SUCCESS, health OK, bundle verificado. |
| 2026-09-16 | `staging` → `main` (pendiente de promoción) | Por asignar | **No** | **Aula / rúbricas:** expone como acciones principales `Crear manualmente`, `IA externa` e `IA de Edusyn`. La vía manual parte de una plantilla editable, usa la escala institucional y conserva la revisión antes de publicar. Solo cliente web; no altera notas, datos académicos ni Prisma. Compilación API y web verificada antes de promoción. |
