...
|-------|---------|--------|-----------|--------|
| 2026-09-24 | **producción** | `aeef893e` (registro en `main` `065153c7`) | **Sí**, dos aditivas | [PR #49](https://github.com/lcardenas7/Edusyn/pull/49): Aula móvil, avisos, períodos, filtros y estado de entregas, CREA y Classroom B1 **17/98**. PostgreSQL 17 local con 98 migraciones; API 67 suites/1250 pruebas, web 35/500, CREA Preview 119, tipos y builds verdes. Staging probó publicación/retirada del aviso en 8C, lectura de entrega existente, contador 37/37 y preview CREA sin alerta. Railway informó éxito para API, web, CREA Preview y CREA Apps en `aeef893e`; los cuatro endpoints públicos respondieron 200, API `health` devolvió `ok` y el preview mantuvo CSP restrictiva. Sin nueva entrega artificial ni cambio de datos académicos en producción. **Seguimiento:** smoke autenticado de producción, rendimiento móvil del bundle principal de 5,9 MB y Classroom 81 rutas aún pendientes. |
| 2026-09-24 | staging | `5d287fa9` | No | **Estado real de una entrega en el detalle.** En la sesión de estudiante de 8C, la lista decía «Entregada» y el detalle «Pendiente» porque `getActivity` no incluía la entrega que `getMySubmission` traía por separado. La respuesta propia ahora alimenta el cálculo del estado; un 404 de entrega no conserva una fila obsoleta. Web 41 archivos/532 pruebas, tipos y build verdes. Los cuatro servicios de Railway informaron éxito; tras recargar el mismo detalle, el encabezado, la línea de tiempo y el panel concordaron en **«Entregada»**. No se cambió el trabajo del estudiante. |
| 2026-09-23 | staging | `8bc12c85` (registro `9ff586f5`) | No | **CREA / aviso falso de recurso faltante.** Un proyecto antiguo conservaba `<link href="styles.css">` en su HTML; el preview ya inyecta el CSS de esa pestaña y el enlace duplicado producía una alerta de recurso inexistente aunque la app renderizaba. Se omiten solo referencias locales exactas a `styles.css` y `app.js` en el documento renderizado, sin modificar los archivos del estudiante ni silenciar fallos de otros recursos. CREA Preview: 5 archivos/119 pruebas y build verdes. Los cuatro servicios de Railway informaron éxito en `9ff586f5`; al volver a abrir el mismo proyecto de 8C, la app renderizó y desapareció la alerta. |
| 2026-09-23 | staging | `ee5e5dce` | No | **Avisos de actividad despublicada.** La prueba autorizada en 8C creó, publicó y devolvió a borrador una tarea técnica sin entregas ni calificaciones. La revisión detectó que el aviso podía quedar en la bandeja apuntando al borrador. Al despublicar, la retirada de destinatarios se programa después del commit; republicar puede volver a avisar. API 114 suites/2435 pruebas, tipos y Nest verdes. Los cuatro servicios de Railway informaron éxito. Se repitió el ciclo publicar/despublicar; la actividad quedó en borrador y, con una sesión de estudiante matriculado en 8C, la bandeja mostró **0 mensajes** y la búsqueda de la actividad mostró **0 resultados**. CREA abrió un proyecto existente del equipo y renderizó su vista previa aislada; mostró una alerta de recurso faltante que se diagnosticó como referencia duplicada a `styles.css`. No hubo entregas ni notas de prueba. |
| 2026-09-23 | staging | `9d9675d6` (registro `677e1ef9`) | No | **Contador de entregas del Aula.** En 8C la tarjeta de un quiz decía «40 de 37 entregaron» mientras el detalle mostraba 37 estudiantes distintos: la tarjeta sumaba intentos. El listado docente ahora calcula matrículas activas distintas del grupo/año institucional y el web prefiere ese conteo. API 114 suites/2432 pruebas, web 41/530, tipos API/web limpios; filtro Entregaron/Todos/Sin entregar comprobado de forma read-only en la sesión docente. Railway informó éxito para los cuatro servicios en `677e1ef9`; tras recargar 8C, la tarjeta mostró **37 de 37**, igual que el detalle. No se cambió ninguna nota, entrega ni matrícula. |
| 2026-09-23 | staging | `e9d06320` (registro `84db2558`) | **Sí** (`20260923010000_aviso_actividad_publicada`, aditiva) | **Aula móvil, avisos, períodos y entregas + Classroom B1 17/98.** Publicación y listado de estudiantes con cadena institucional; avisos solo después del commit; entregas por apellido con filtros Entregaron/Todos/Sin entregar; advertencia al quedar vacío un período; estilo interior actualizado. API 114 suites/2431 pruebas, web 41/526, CREA 11, tipos y builds verdes. PostgreSQL local desechable: 105 migraciones/0 fallidas con roles RLS. Railway informó éxito para los cuatro servicios de staging en `84db2558`; API, web y CREA apps respondieron 200. La sesión docente de 8C permite recorrer filtros y cambio de período; publicación/entrega integral con datos controlados aún pendiente antes de producción. Classroom sigue parcial. |
| 2026-09-23 | staging | `f14f6e4d` | **No** | **CREA / ciclo de vida de apps:** vencimiento por defecto 30 días después del cierre lectivo (si falta fecha, el docente la indica); retiro con advertencia de copia sin conexión; eliminación manual de la publicación y telemetría con nombre exacto y resumen agregado en bitácora; `crea-apps` falla cerrado si la API cae y acota la caché positiva a 2 s. API 107 suites/2201 pruebas, web 478, `crea-apps` 11, tipos y builds verdes. Push confirmado; los endpoints de salud de API, web y `crea-apps` respondieron 200. **Falta acreditar el hash desplegado por Railway y el recorrido autenticado** antes de promover a producción. |
| 2026-09-23 | staging | c415782a | **Sí** (`20260923010000_edusyn_crea_permiso_ia`, aditiva: `ConstruyeProject.aiPromptEnabled` DEFAULT true) | **Crea / el docente autoriza la IA:** interruptor por proyecto; con la IA apagada el taller oculta el momento «Prompt» y «Pedir un cambio a la IA». Incluye `a74e5059`: prompts al día (se publica con QR e instalable, funciona sin internet, datos en localStorage, sin cuentas ni datos personales) y reglas de acabado concretas (color de acento, 8px, 16px base, 44px al tocar, estado vacío amable, foco visible); la petición de cambio distingue web/app. API 2.197, web 478. |
| 2026-09-23 | **producción** | 08a4627a | **Sí** (`20260921010000_edusyn_crea_kind_code_draft` + `20260922010000_edusyn_crea_publicacion`, aditivas) | **Crea completo a producción:** estudio tipo MakeCode, selector web/app, editor CodeMirror con guardado automático del código, publicación con enlace/QR instalable y medición de adopción, revisión del docente a pantalla completa, y prompts al día. **Servicio nuevo `crea-apps`** (https://crea-apps-production.up.railway.app, variables `EDUSYN_API_URL`, `PORT`, `RAILPACK_*`; la API lleva `CREA_APPS_ORIGIN`). Verificado: API 1.008, web 443, crea-apps 10, tipos y build limpios. |
| 2026-09-22 | staging | 18474c58 | **Sí** (`20260922010000_edusyn_crea_publicacion`, aditiva: `ConstruyePublication`, `ConstruyeAppDevice`, `ConstruyeAppDay`, enum `ConstruyePublicationStatus`, valor `PUBLICATION` en `ConstruyeJournalType`) | **Crea / publicar la app (entrega 1):** el equipo pide publicar una versión, el docente aprueba y la app queda en `/a/<token>/` con QR, instalable en Android e iPhone y funcionando sin conexión; medición de uso anónima (personas, instalaciones, activos, retención, origen) para estudiante y docente; «Ver la app» del docente ahora a pantalla completa y en proporción. **Servicio nuevo en Railway: `crea-apps-staging`** (https://crea-apps-staging-production.up.railway.app, variables `EDUSYN_API_URL`, `PORT=5175`, `NIXPACKS_*`; la API lleva `CREA_APPS_ORIGIN`). API 2.195, web 471, crea-apps 10. Doc: `docs/DISENO_CREA_PUBLICAR_Y_DATOS.md`. **Pendiente de revisión del usuario antes de producción.** |
| 2026-09-21 | staging | 45243985 | **Sí** (`20260921010000_edusyn_crea_kind_code_draft`, aditiva: enum `ConstruyeProjectKind`, `ConstruyeProject.kind` DEFAULT WEB, `ConstruyeTeam.codeDraft*`) | **Crea / web o app + guardado del código:** selector docente «Página web / Aplicación para celular» (plantilla de app, vista celular, prompt de app), editor CodeMirror, guardado automático del código con guarda de revisión (409 → «Cargar lo suyo / Mantener lo mío»), recuperación del borrador, «Descargar» .html, localStorage en memoria dentro del sandbox del preview. Rutas nuevas `PATCH construye/projects/:id`, `PUT construye/teams/:id/code-draft`. API 2.182, web 471, preview 117. **Pendiente de revisión del usuario antes de producción.** |
| 2026-09-21 | staging | b077785d | **No** | **Crea / estudio tipo MakeCode:** barra con Documentar · Prompt · Código · Presentar, guardado automático del plan (estado real, reintento, aviso al cerrar), prompt guiado con «Omitir», taller con vista previa a la izquierda + código a la derecha + barra inferior y paneles en cajón; el taller no se desmonta al cambiar de pestaña. API: `PATCH brief` acepta `fields` (merge por campo entre compañeros) y agrupa la racha de edición en una entrada de bitácora (20 min). API 2.178, web 466. **Pendiente de revisión del usuario antes de producción.** |
...
| 2026-09-16 | staging | 7c95e371 | **No** | **Aula / coevaluación:** antes de publicar, reparto de quién evalúa a quién automático (equilibrado, con semilla, `GET formative-evaluations/:id/peer-preview`) o manual (`publish` acepta `peer`). El destino en la planilla se elige solo al enviar resultados (opcional). Agregar coevaluación copia criterios y cada dimensión puede traerse de IA externa (la IA propone `peersPerStudent`). Botón directo "Crear con IA externa" en Hoy/Actividades y aula a ancho completo. API 105/2.165, web 454, tsc y build OK. |
| 2026-09-16 | staging | Por asignar | **No** | **Aula / rúbricas:** expone como acciones principales `Crear manualmente`, `IA externa` e `IA de Edusyn`. La vía manual parte de una plantilla editable, usa la escala institucional y conserva la revisión antes de publicar. Solo cliente web; no altera notas, datos académicos ni Prisma. Compilación API y web verificada antes de promoción. |
| 2026-09-12 | staging | 080860e5 | No | Taller: 24 pruebas HTTP nuevas con JWT/guards reales para las diez rutas institucionales A→B/B→A, además de catálogo, ausencia de JWT y mismo usuario en dos colegios. Focal 64/64; API completa 103/2.123; tipos API/web y Nest aprobados. No cierra Taller: faltan cadenas históricas equipo/proyecto/aula y miembro/matrícula y atomicidad general. Sin base compartida, RLS ni producción. |
| 2026-09-12 | staging | 97f85691 + a43db832 + 4fd6d9c4 | No | Teacher Schedule 4/4 y Staff Leave 7/7 cerrados en aplicación. Agenda personal usa institución + actor, transacciones reales y RolesGuard. Permisos laborales protegen PII, separan alcance personal/administrativo, validan relaciones y fechas, y vuelven atómicas revisión/cancelación. 106 pruebas nuevas; focal con contrato 119/119; suite completa 102/2.099, tipos API y Nest aprobados; siete excepciones retiradas aparte. Classroom queda medido 0/98. La continuidad inicial Observer → Classroom B1 fue reemplazada el mismo día por la coordinación versionada Claude: Observer → PostgreSQL; Kimi: Classroom B1. Sin RLS, producción ni datos reales. |
| 2026-09-12 | staging | d50bce46…ddb1f058 | No | Attendance integrado y revisado: 17/17 rutas directas, 16 excepciones retiradas, institución del actor, relaciones completas y 404 cruzado. La revisión 70726854 corrigió tres operaciones que ignoraban el cliente `tx`, volvió atómicas la asistencia y su auditoría, y cambió el contexto ausente de 500 a 400 en el resolvedor común. 68 pruebas de servicio + 56 HTTP; mutación y cliente raíz prohibido comprobados. Total 98 suites / 1.993 API, web 22/214, tipos API/web y Nest aprobados. Sin migración, RLS, producción ni datos reales. Observer queda encargado en documento versionado. |
| 2026-09-11 | staging | 2022be74 + 172cd375 | No | Cortes preventivos: 8/8 rutas con institución del actor; cadena período/año, grupo/sede/grado, asignación/asignatura, matrícula/estudiante y alerta acotada. Ejecución del lote atómica y motor compartido de nota a fecha institucional. DTOs aceptan CUID. 38 pruebas de servicio + 46 HTTP; focal con contrato 97, mutación detectada. Suite completa 96/1.869 API, tipos API/web y Nest aprobados; web 22/214 verificado en la sesión. Ocho excepciones retiradas en commit separado. PostgreSQL/RLS, Bloque 4 y reglas históricas de roster pendientes. |
| 2026-09-11 | staging | eaa57408 | No | Learning Route integrado: 14 rutas institucionales resuelven la institución del actor y validan ruta→aula, paso→ruta y actividad→aula; 2 rutas no institucionales quedan justificadas. La revisión corrigió cuatro transacciones que usaban el cliente Prisma exterior, revalidó dentro de `tx` y acotó evidencia idempotente. 52 pruebas de servicio + 40 HTTP; focal con contrato 105. Suite completa 94/1.785 API, web 22/214, tipos API/web y Nest aprobados. PostgreSQL/RLS, cuota IA y Bloque 4 pendientes. Attendance queda habilitado como siguiente frente en su encargo versionado. |
...
| 2026-09-11 | staging | 9ae32d6b, ff81a71b | No | Inclusión: acceso real de docente/orientación al contexto de trabajo, 3 consultas APD propias, perfiles/planes acotados y coherencia matrícula/período/perfil. 57 pruebas nuevas; API 88 suites / 1.440 pruebas, web 22 archivos / 214, tipos API/web, Nest y navegador sintético aprobados. Push ee1cbb3a..ff81a71b confirmado. Auditoría APD explícitamente parcial; ver AUDITORIA_AISLAMIENTO_INCLUSION.md. Railway pendiente de verificar, sin producción. |
| 2026-09-11 | `staging` | `7e2a1b7a` | **No** | **Foro: vista previa legible y HTML de usuario siempre limpio.** Solo cliente web, cero migraciones. (1) La lista del foro mostraba el HTML crudo del editor (`<p>Responde&nbsp;con…</p>`, `<span style=…>`): la vista previa pasa a texto plano extraído con `DOMParser` (documento inerte). (2) **Hallazgo de seguridad:** `RichContent` insertaba el HTML de usuario SIN limpiar y la API tampoco lo limpia; en el foro escriben estudiantes, así que un post con `<img src=x onerror=…>` se ejecutaba en el navegador del docente que lo abría (XSS almacenado, presente en producción en ambas aulas). Ahora pasa por DOMPurify, que conserva el formato del editor y quita lo ejecutable; los enlaces con `target` llevan `rel=noopener noreferrer`. Verificado en navegador real: 7 cargas de ataque → 0 ejecuciones; sin limpiar, las mismas SÍ se ejecutan. DOMPurify ya estaba en el lock como dependencia indirecta (3.3.1): se declara directa con la misma versión. 214/214 pruebas web, `tsc` limpio, build correcto; el paquete servido contiene la limpieza. **Pendiente:** promover a `main`. La API sigue sin limpiar al guardar: defensa adicional recomendada, no bloqueante. |
| 2026-09-11 | staging | 85b321ed, 8f63c9d9 | No | Matrículas: aislamiento y movimientos atómicos, 111 pruebas nuevas; mejoras de filtros/edición e Inclusión con agenda. API 1383 pruebas, web 208, tipos, Nest y navegador sintético aprobados. Push confirmado; Railway pendiente de verificar. Sin producción. |
| 2026-09-10 | staging | 000d435b | No | Taller: 13 operaciones posteriores acotadas, guardas compartidas y 40 pruebas nuevas de aislamiento. 84 suites / 1.272 pruebas API, tipos API y Nest correctos; web sin cambios (203 pruebas vigentes). Diez rutas con requireInstitutionId y catálogo estático exceptuado. Push confirmado; Railway y laboratorio HTTP pendientes. Incluye registro del push afe9f388. Sin bases de datos, migraciones ni producción. |
| 2026-09-10 | staging | afe9f388 | No | Bloque 0: contrato estructural de institución en todas las rutas, excepciones exactas con huella y deuda separada. 83 suites / 1.232 pruebas API, 203 web, tipos API/web y Nest correctos. ESTADO_BLINDAJE con 39 módulos; no equivale a aislamiento cerrado. Push confirmado; estado de Railway pendiente de verificar. Sin conexiones a bases ni promoción a producción. |
| 2026-09-09 | `staging` | `5c24784b` | **No** | **Identidad institucional unificada en formatos.** Actas y seguimientos ahora leen el escudo del perfil institucional y, si una institución todavía lo tiene solo en la configuración histórica de Boletines, usan ese valor como respaldo. Al guardar el escudo o el color desde Boletines, se consolidan en el perfil para que todos los formatos posteriores usen la misma fuente. Prueba de respaldo aprobada, 6 pruebas de exportación de Observador y tipos API limpios. Sin migración ni escrituras de datos reales. |
| 2026-09-09 | `staging` | `e014360d` | **No** | **Blindaje A/B del modulo de Recuperaciones completo.** Cero migraciones, cero esquema, solo `apps/api/src/modules/recovery/`. Auditoria multiinstitucion del modulo entero: **21 de 33 rutas** recibian un identificador del cliente y operaban SIN resolver ni comprobar la institucion; nueve de ellas escriben. Ahora **0 sin contexto**. El defecto se repetia en los cuatro servicios: la institucion se deducia del RECURSO que nombraba el cliente y no del actor, asi que la fila quedaba coherente y **ninguna auditoria de datos podia detectarlo**. Permitia, entre otras cosas: regenerar boletines y finalizar el proceso de recuperacion de un periodo ajeno; reescribir un plan de refuerzo ajeno (`updatePlan` actualizaba por id sin cargar el registro); aprobar un acta academica ajena; y cambiarle a otra institucion la nota maxima que puede sacar un estudiante recuperando. Dos rutas PARECIAN protegidas (`registerResult`, `reviewResult`): recibian `institutionId` pero solo lo usaban para elegir las reglas. Reglas aplicadas: la institucion la pone el actor; toda consulta la lleva en el `where`; `NotFoundException` y nunca `Forbidden`; `deleteMany` acotado en vez de `delete` por id; ningun parametro de institucion opcional. **30 pruebas de rechazo cruzado A/B**, que afirman lo que NO llego a ocurrir. Verificado: **82 suites / 1218 pruebas API**, `tsc --noEmit` limpio, `nest build` correcto. No se creo, edito ni borro ningun dato real. Detalle y limites en `docs/AUDITORIA_AISLAMIENTO_RECUPERACIONES.md`. **Pendiente:** reproduccion HTTP cruzada real con dos instituciones sinteticas y autorizacion por asignacion docente (§6 del documento). **NO promovido a produccion.** |
...
| 2026-09-09 | `staging` | `a8d1f457` | **No** | **Descarga institucional de seguimientos pedagógicos.** Las observaciones `PEDAGOGICAL_FOLLOWUP` ahora generan un informe PDF individual por estudiante, con situación identificada, acciones y apoyos, plan y valoración, comunicación con acudiente, estado y firmas. La lista permite seleccionar varios y descarga un único PDF multipágina; el detalle permite descarga directa. Reutiliza identidad institucional, aislamiento por tenant y límites de rol de las actas. Verificado: 55 pruebas del Observador, TypeScript API/web y muestra visual de una página. Sin migración ni escrituras de datos. |
