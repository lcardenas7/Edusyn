...
### Paso 1 · Capa de gamificación `LearningIdentity` — increment 1 HECHO (2026-07-04)
- **Modelo de datos (aditivo, migración `20260704140000_learning_identity_gamification`):** `LearningIdentity` (anclada en `Student` → persiste entre años, para el portafolio longitudinal) con `totalXp`, `level`, `currentStreak`/`longestStreak`, `skillXp` (JSON por materia). Ledger `XpEvent` **idempotente** (`idempotencyKey` único) para no duplicar concesiones. Enum `XpSource`. RLS defensivo (mismo patrón tenant que el resto).
...
### Paso 2 · Rutas de Aprendizaje + grafo CEFR (2026-07-07)
Arranca el corazón del wedge bilingüe. Por increments verificables:
- **Increment 1 — grafo CEFR canónico:** `model Competency` (referencia GLOBAL, no tenant-scoped; agnóstico de framework para CEFR/DBA/Saber). Seed `prisma/seed-cefr.ts`: **48 can-do statements** A1–B2 × Reading/Listening/Speaking/Writing. Resuelve el cold-start del grafo (crítica A.3 de la Visión). Aplicado y sembrado en staging.
- **Increment 2 — `LearningRoute` + `LearningRouteStep`:** la ruta es un contenedor de pasos que converge en una competencia; cada paso ES una `ClassroomActivity`. Tenant-scoped (RLS). `LearningRouteService` + controller (`/learning-routes`): selector de competencias, CRUD de rutas/pasos, reordenar. CRUD verificado en caliente en staging (ruta objetivo A2 + pasos enganchados a can-do's + cascada).
- **Increment 2b — UI del constructor de rutas:** pestaña "Rutas" en el aula. Docente crea rutas con competencia objetivo (selector del grafo CEFR), añade/elimina pasos, publica. Estudiante ve rutas publicadas. Mapa de pasos → competencia.
- **Increment 3a — motor de evidencia:** `CompetencyEvidence` (tenant-scoped) + `CompetencyEvidenceService` (recordFromActivity idempotente, getMastery = promedio de 3 mejores, getRouteProgress). Cableado a submitQuiz/gradeSubmission/lección: cada actividad enganchada a un paso de ruta con can-do produce evidencia → deriva "% dominado". Verificado en staging (0%→85%, demostrado, idempotente). El "% dominado" de la maqueta ya es real.
- **Increment 3b — UI de progreso del estudiante** (barra "% dominado" + estado por paso) y **enganche paso→actividad** desde la UI.
- **Increment 3c — "Armar ruta con Valeria":** `ApdAiService.generateRoutePlan` + `createFromPlan` (mapea a competencias del grafo). UI botón + preview. Verificado con IA real (objetivo → ruta A2 Speaking, 4 pasos R/L/S/W).
- **Increment 3d — actividad propia de la ruta:** `ClassroomActivity.isRouteScoped` (oculta de la pestaña Actividades); `addStepWithNewActivity` la crea + el paso en un paso. Una Tarea de ruta ES el componente Writing. UI "Añadir paso" con modos "crear para la ruta" / "enlazar existente".
- **Increment 3e — el estudiante hace la actividad desde la ruta:** botón "Hacer"/"Ver" por paso; Writing/Tarea inline (StepActivityModal). **Loop completo verificado en staging:** crear Writing → entregar → calificar → evidencia → dominio 90% demostrado.
- **Increment 3f — el paso ES una lección interactiva de inglés (estilo Duolingo), generada por Valeria** (decisión del fundador). `ApdAiService.generateEnglishLessonSlides({skill,level,objective,title})` produce ejercicios por habilidad CEFR (Reading con texto, Listening con guion, Writing con construcción de frases) usando MULTIPLE_CHOICE/TRUE_FALSE/FILL_BLANK/ORDERING; reusa el motor de Lecciones (gamificación + evidencia). `generateStepLesson` crea la actividad LESSON propia de la ruta + guarda las slides. UI: docente "Generar/Regenerar con Valeria" por paso; estudiante "Hacer" abre el `LessonPlayer`. Verificado con IA real: paso Reading A2 → texto en inglés + 4 ejercicios (MC, V/F, completar, ordenar). **Así se llenan los pasos "sin actividad" que Valeria dejaba vacíos.**
- **Pendiente:** que "Armar con Valeria" genere las lecciones de todos los pasos automáticamente (hoy es por paso, por la latencia del tier free); Listening con audio TTS real; Speaking (motor de voz, Fase 1).
